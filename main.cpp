...
void SERIAL_begin()
{
	unsigned int ubrr = 51; // (freq / baud / 16 - 1) : 19200bps=51
...
void SERIAL_print(char const *p)
{
	while (*p) {
		SERIAL_write(*p);
		p++;
...
{
	if (CRC_EOP == getch()) {
		SERIAL_print((char)STK_INSYNC);
		SERIAL_print((char)STK_OK);
	} else {
		ISPError++;
		SERIAL_print((char)STK_NOSYNC);
	}
}
...
void write_flash(int length)
{
	...
	if (CRC_EOP == getch()) {
		SERIAL_print((char)STK_INSYNC);
		SERIAL_print((char)write_flash_pages(length));
	} else {
		ISPError++;
		SERIAL_print((char)STK_NOSYNC);
	}
...
char flash_read_page(int length)
{
	for (int x = 0; x < length; x += 2) {
		...
		SERIAL_print((char)low);
		uint8_t high = flash_read(HIGH, here);
		SERIAL_print((char)high);
		here++;
	}
	return STK_OK;
}
